          ...
          <div className="border-t border-navy/25">
            {faqs.map((faq, index) => (
              <details key={faq.question} className="group border-b border-navy/20">
                <summary className="flex cursor-pointer list-none items-start gap-5 py-7 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#00815f] sm:gap-8 sm:py-8">
                  <span className="mt-1 text-xs font-black tracking-[0.12em] text-[#7a8c9c]">0{index + 1}</span>
                ...
                <p className="ml-10 max-w-3xl pb-8 text-sm leading-7 text-ink-muted sm:ml-[68px] sm:text-base">{faq.answer}</p>
              </details>
            ))}
          </div>
        </div>
