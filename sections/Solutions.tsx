...
function SolutionCard({ solution, index, large = false }: { solution: (typeof primary)[number]; index: number; large?: boolean }) {
  return (
    <article data-reveal className={`interactive-card group relative isolate overflow-hidden rounded-[24px] border border-navy/16 bg-navy shadow-[0_18px_50px_rgba(0,20,42,.12)] ${large ? "min-h-[590px]" : "min-h-[286px]"}`}>
  ...
  return (
    <section id="solucoes" className="bg-paper py-20 text-navy sm:py-24 lg:py-28">
      <Container>
        ...
        <div data-reveal className="mt-8 grid overflow-hidden rounded-[22px] border border-navy/16 bg-white/25 shadow-[0_16px_44px_rgba(0,20,42,.06)] lg:grid-cols-[.55fr_1.45fr]">
          <div className="p-6 lg:border-r lg:border-navy/16 lg:p-8">
          ...
          <div className="grid sm:grid-cols-3">
            {technical.map((service, index) => (
              <Link key={service.title} href={service.href} className="group relative border-t border-navy/16 p-5 transition-[background-color] hover:bg-white/60 sm:border-l sm:border-t-0 sm:p-6">
